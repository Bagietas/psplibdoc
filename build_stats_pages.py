...
def make_stats(module, lib, version, obfuscated, cur_nids, prev_nonobf, prev_ok, nid_names, obfuscation_pairs):
    unk_nids = []
    nok_nids = []
    ok_nids = []
    obf_nids = []
    ...
    for cur_nid in cur_nids:
        ...
        if fullname in nid_names:
            ...
            else: # TODO: handle guesses
                unk_nids.append({"nid": cur_nid, "name": nid_names[fullname][0], "source": nid_names[fullname][1]})
        else:
            obf_nids.append({"nid": cur_nid, "name": fullname, "source": 'obfuscated'}) # TODO: differentiate cases where the previous NID is known or not, and if its name was found or not

    stats = {"known": ok_nids, "unknown": unk_nids, "wrong": nok_nids, "unknown_obf": obf_nids}
...
def handle_library(module, lib, nids, versions, nid_names, obf_pairs):
    ...
    prev_nonobf = {}
    prev_ok = {}
    stats_byver = {vers[0]: (make_stats(module, lib, vers[0], now_obfuscated, get_nids_ver(nids, vers[0]), prev_nonobf, prev_ok, nid_names, obf_pairs), False)}
    for (v1, v2) in zip(vers, vers[1:]):
        # For each consecutive firmware versions v1 and v2, see their respective NIDs
        is_obfuscated = False # TODO
        if is_obfuscated:
            now_obfuscated = True
        stats_byver[v2] = (make_stats(module, lib, v2, now_obfuscated, get_nids_ver(nids, v2), prev_nonobf, prev_ok, nid_names, obf_pairs), is_obfuscated)
...
def main():
    ...
    for line in open('obfuscation_pairs.csv', 'r').read().split('\n')[:-1]:
        obfuscation_pairs[line.split(',')[0]] = line.split(',')[1]
